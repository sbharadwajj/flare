...
def image_loss_fn(img, target, mask, loss, tonemapper):
    ...
    ## to avoid the network learning that extra white halo
    # mask_gt = ((mask > 0.1).int().bool()).repeat(1, 1, 1, 3)
    ...
    if loss == 'mse':
        ...
        # return torch.nn.functional.l1_loss(img[mask_gt], target[mask_gt]), img, target
        return torch.nn.functional.l1_loss(img, target), img, target